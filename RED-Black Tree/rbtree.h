...
public:
    ...
    RedBlackTreeIterator &operator++()
    {
        ...
        if (node_ptr == nullptr)
        {
            ...
            if (node_ptr == nullptr)
                throw tree_exception("RedBlackTreeIterator operator++(): tree empty");
...
public:
    ...
    {
        Node<K, V> *node = new Node<K, V>(key_value.first, key_value.second);
        Node<K, V> *temp = nullptr;
        Node<K, V> *iterNode = it != end() ? it.tree->root_ : root_;

        while (iterNode)
        {
            temp = iterNode;

            if (key_value.first == iterNode->key())
            {
                delete node;
                std::stringstream ss;
                ss << key_value.first;
            ...
            iterNode = key_value.first < iterNode->key() ? iterNode->left : iterNode->right;
        }
...
private:
    ...
    void insert_fixup(Node<K, V> *z)
    {
        while (z != root_ && z->parent->color == RED)
        {
            ...
            Node<K, V> *uncle = isLeft ? z->parent->parent->right : z->parent->parent->left;
            ...
            else
            {
                ...
                {
                    z->parent->parent->color = RED;
                    isLeft ? right_rotate(z->parent->parent) : left_rotate(z->parent->parent);
                }
            }
        }
    ...
    void left_rotate(Node<K, V> *x)
    {
        ...
        else
        {
            if (x == x->parent->left)
            {
                x->parent->left = rightNode;
            }

            else
            {
                x->parent->right = rightNode;
            }
        }
    ...
    void right_rotate(Node<K, V> *x)
    {
        ...
        Node<K, V> *leftNode = x->left;
        x->left = leftNode->right;

        if (leftNode->right)
        {
            leftNode->right->parent = x;
        }
        ...
        else
        {
            if (x == x->parent->right)
            {
                ...
                x->parent->right = leftNode;
            }

            else
            {
                x->parent->left = leftNode;
            }
        }
    ...
    size_t leaf_count(Node<K, V> *node) const
    {
        ...
        size_t countLeft = leaf_count(node->left);
        size_t countRight = leaf_count(node->right);
    ...
    int diameter(Node<K, V> *node) const
    {
        ...
        int longestPathThroughRoot = leftHeight + rightHeight + 2;
        int maxDiameterSubtree = std::max(leftDiameter, rightDiameter);

        return std::max(longestPathThroughRoot, maxDiameterSubtree);
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const
    {
        ...
        size_t left_sum = sum_null_levels(node->left, level + 1);
        size_t right_sum = sum_null_levels(node->right, level + 1);
